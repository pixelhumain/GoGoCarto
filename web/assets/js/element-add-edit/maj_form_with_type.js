...
{
	//window.console.log("maj_form_with_type val = " + $('#type_element').val());
	if ($('#type_element').val() == '0') return; 

	init = init !== false;

	if (first_maj_form_with_type_done) window.open('add?type='+ $('#type_element').val(),'_self');
	else
	{
