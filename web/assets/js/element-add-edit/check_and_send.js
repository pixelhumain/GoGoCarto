...
{	
	// CHECK type element
	if (!$('#type_element').val() || $('#type_element').val() == '0') 
	{
		$('#label_type_element').addClass('error'); 
		$('#label_type_element').text('Veuillez choisir un type de element');	
	}
	else 
	{
		$('#label_type_element').removeClass('error');
	}
	...
	else 
	{
		...
		$('#title_products').text('Produits du element'); 		
	}
	...
	if (nbre_erreur === 0) 
	{
		...
		if (( $('#type_element').val() == "2") && ( $('.ouvert').length > 1))
		{
